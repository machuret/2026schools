...
import { type AreaIssue, INPUT_CLS, INPUT_STYLE, LABEL_CLS, LABEL_STYLE, SEVERITY_LEFT } from "./AreaTypes";

function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return <div className="mb-4"><label className={LABEL_CLS} style={LABEL_STYLE}>{label}</label>{children}</div>;
}

export default function AreaIssueCard({ issue, idx, onChange, onRemove }: {
